...
/// <summary>
/// Lazy singleton <see cref="McpClient"/> to the Context7 MCP
/// (SPEC-20260922-context7-mcp-proxy RF-002) — same shape as
/// <see cref="TavilyUpstreamClient"/>: AutoDetect transport (Streamable HTTP,
...
/// (DB) first, then <see cref="Context7Options.ApiKey"/> (env/config). Bearer is
/// used instead of the CONTEXT7_API_KEY header variant so the key never
/// lands in URLs/access logs.
/// </summary>
public sealed class Context7UpstreamClient(
    IOptions<Context7Options> options,
    IIntegrationSecretStore secrets,
    ILoggerFactory loggerFactory,
    ILogger<Context7UpstreamClient> logger) : IAsyncDisposable
{
        ...
        string? apiKeyOverride = null)
    {
        ...
        {
            logger.LogInformation("context7 call failed ({Message}); reconnecting once", first.Message);
            await ResetAsync();
            try
            {
                return await InvokeAsync(toolName, dict, cancellationToken, apiKeyOverride);
            ...
            {
                return ErrorResult($"context7 upstream error: {second.Message}");
            }
        }
    }
    ...
    private async Task<McpClient> GetClientAsync(CancellationToken cancellationToken, string? apiKeyOverride = null)
    {
        ...
        try
        {
            ...
            _client = await McpClient.CreateAsync(
                ...
                {
                    ClientInfo = new Implementation { Name = "knowledge-hub-context7-proxy", Version = "0.1.0" }
                },
                loggerFactory,
                cancellationToken);
