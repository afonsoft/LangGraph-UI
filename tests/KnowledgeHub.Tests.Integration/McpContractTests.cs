...
public class McpContractTests : IClassFixture<McpContractTests.Fixture>
{
    ...
    private static readonly Dictionary<string, string> PinnedSchemas = new()
    {
        ...
        ["find_dependencies"] = """{"type":"object","properties":{"component":{"type":"string","description":"Entity name (service, database, API, team…)","examples":["payments-api"]},"depth":{"type":"integer","description":"Traversal depth 1-3 (default 1, clamped to 3)"}},"required":["component"],"examples":[{"component":"payments-api","depth":2}]}""",
        ["find_dependents"] = """{"type":"object","properties":{"component":{"type":"string","description":"Entity name (service, database, API, team…)","examples":["payments-api"]},"depth":{"type":"integer","description":"Traversal depth 1-3 (default 1, clamped to 3)"}},"required":["component"],"examples":[{"component":"payments-api","depth":2}]}""",
        ["find_path"] = """{"type":"object","properties":{"a":{"type":"string","description":"Source entity name"},"b":{"type":"string","description":"Target entity name"},"depth":{"type":"integer","description":"Max path length 1-3 (default 3, clamped)"}},"required":["a","b"],"examples":[{"a":"web-frontend","b":"orders-db","depth":3}]}""",
        ["analyze_impact"] = """{"type":"object","properties":{"component":{"type":"string","description":"Entity name whose dependents are analyzed"}},"required":["component"],"examples":[{"component":"payments-api"}]}""",
