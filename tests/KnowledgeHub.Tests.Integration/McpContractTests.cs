...
public class McpContractTests : IClassFixture<McpContractTests.Fixture>
{
    ...
    private static readonly Dictionary<string, string> PinnedSchemas = new()
    {
        ...
        ["set_api_key_settings"] = """{"type":"object","properties":{"provider":{"type":"string","enum":["chat","firecrawl","tavily"],"description":"Which provider to configure"},"endpoint":{"type":["string","null"],"description":"OpenAI-compatible base URL (chat only, null = inherit)"},"model":{"type":["string","null"],"description":"Model name (chat only, null = inherit)"},"apiKey":{"type":["string","null"],"description":"API key override (null = inherit from global)"}},"required":["provider"],"examples":[{"provider":"chat","endpoint":"http://localhost:11434","model":"llama3","apiKey":null}]}""",
