...
public class McpContractTests : IClassFixture<McpContractTests.Fixture>
{
    ...
    private static readonly Dictionary<string, string> PinnedSchemas = new()
    {
        ...
        ["agent_chat"] = """{"type":"object","properties":{"prompt":{"type":"string","description":"Pergunta/tarefa em linguagem natural — o agente itera tools até responder"},"tools":{"type":"array","items":{"type":"string"},"description":"Allowlist de tools expostas ao modelo (default: todas as read-only)"},"maxIterations":{"type":"integer","description":"Teto de iterações model→tools→model (default 10)"},"allowWrite":{"type":"boolean","description":"Opt-in: expõe tools de escrita (write_knowledge, write_note)"},"threadId":{"type":"string","description":"GUID de thread existente — continua a conversa com contexto"},"persist":{"type":"boolean","description":"Cria thread nova e persiste os turnos desta chamada"}},"required":["prompt"]}""",
